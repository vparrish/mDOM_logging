...
with open(file_path, "w") as file:
    ...
    with open(csv_file_path, "w", newline='') as csv_file:
        ...
        for entry_number in range(1, num_entries + 1):
            ...
            if entry_number == 1:
                link = f"https://wiki.icecube.wisc.edu/index.php/Upgrade_Device_Assembly#half_A"
            else:
                link = f"https://wiki.icecube.wisc.edu/index.php/Upgrade_Device_Assembly#half_A_{link_number}"
            csv_writer.writerow([link])

print(f"Files '{file_path}' and '{csv_file_path}' have been created with {num_entries} entries and links (without headers).")